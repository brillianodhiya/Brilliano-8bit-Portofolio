...
export function Navigation() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      {/* Top Status Bar */}
      <div className="bg-background border-b-4 border-white h-12 flex items-center px-4 justify-between pixel-shadow relative">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="font-display text-[10px] text-secondary text-shadow-pixel">LVL</span>
      ...
      <nav className="flex justify-center -mt-1 relative z-10">
        <div className="pixel-panel border-t-0 p-1 flex gap-1 sm:gap-2 flex-wrap justify-center max-w-[95vw]">
          {NAV_ITEMS.map((item) => {
